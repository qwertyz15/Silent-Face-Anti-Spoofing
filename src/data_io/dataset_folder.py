...
class DatasetFolderFT(datasets.ImageFolder):
    ...
    def __getitem__(self, index):
        try:
            ...
            ft_sample = generate_FT(sample)
            ft_sample = cv2.resize(ft_sample, (self.ft_width, self.ft_height))
            ft_sample = torch.from_numpy(ft_sample).float().unsqueeze(0)
            ...
            # Returning dummy data of the same structure and size
            dummy_sample = torch.zeros_like(sample)
            dummy_ft_sample = torch.zeros_like(ft_sample)
            dummy_target = -1  # Placeholder target
            return dummy_sample, dummy_ft_sample, dummy_target
