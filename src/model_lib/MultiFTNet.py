...
class MultiFTNet(nn.Module):
    def __init__(self, img_channel=3, num_classes=3, embedding_size=128, conv6_kernel=(5, 5), model_type='V2'):
        super(MultiFTNet, self).__init__()
        self.img_channel = img_channel
        self.num_classes = num_classes
        if(model_type == 'V1'):
            self.model = MiniFASNetV1(embedding_size=embedding_size, conv6_kernel=conv6_kernel, num_classes=num_classes, img_channel=img_channel)
        elif model_type == 'V1SE':
            self.model = MiniFASNetV1SE(embedding_size=embedding_size, conv6_kernel=conv6_kernel, num_classes=num_classes, img_channel=img_channel)
        elif model_type == 'V2':
        ...
        elif model_type == 'V2SE':
            self.model = MiniFASNetV2SE(embedding_size=embedding_size, conv6_kernel=conv6_kernel, num_classes=num_classes, img_channel=img_channel)
